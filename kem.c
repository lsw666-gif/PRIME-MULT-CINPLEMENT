...
static void Rq_mult_small(Fq* h, const Fq* f, const small* g) {//扩张的NTT乘法加速 typedef int16 Fq;
    //假设f是R/q,g是small
    //预先设定pp是扩张的维数，qq是扩张的模数
    Fq ff[pp];
    Fq gg[pp];
    Fq fg[pp];//存放中间值
    int i = 0;
    for (int i = 0; i < pp; i++) {//复制，然后高位暂时设为0
        if (i >= 0 && i < p) {
            ff[i] = f[i];
            gg[i] = g[i];
        }
    ...
    ntt(ff);
    ntt(gg);
    basemul_montgomery(fg, ff, gg);
    invntt(fg);

    for (i = p + p - 1; i >= p; --i) {
        //原来的prime环 fg[i-p] = Fq_freeze(fg[i-p]+fg[i]);
